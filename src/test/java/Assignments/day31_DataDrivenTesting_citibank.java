...
import day31_datadriventesting.XlUtils;
import io.github.bonigarcia.wdm.WebDriverManager;

public class day31_DataDrivenTesting_citibank {
	...
	public static void main(String[] args)  throws InterruptedException, IOException {
		...
		int rows = XlUtils.getRowCount(file, "Sheet1"); // get a row count from the sheet
		System.out.println("row count is : " + rows);

		for (int i = 1; i <= rows; i++) {
	// reading data from excel
		String inidepo= XlUtils.getCellData(file,"Sheet1",i,0);				//file,sheet,row #, col zero for initial deposit
		String interestrate=XlUtils.getCellData(file,"Sheet1",i, 1);		//interest rate
		String monthlength=XlUtils.getCellData(file,"Sheet1",i, 2);			//length
		String compoundingmonths=XlUtils.getCellData(file,"Sheet1",i, 3);	//compounding String exptotal=XLUtils.getCellData(path,"Sheet1",i, 4);	//expected total , will be compared with actual total
		String exptotal=XlUtils.getCellData(file,"Sheet1",i, 4);	//expected total , will be compared with actual total
	...
	//Dropdown (Boostrap) - Not having Select Tag
		...
		if(exptotal.equals(acttotal)) {			//if expected total = actual total then			
			...
			XlUtils.setCellData(file, "Sheet1",i, 6,"Passed");	//setting passed in 6th column (index start with zero)
			XlUtils.fillGreenColor(file, "Sheet1",i, 6);	//filling the color in 6th column if passed then greeen or faile then red.
		}
		else
		{
